...
case class Product(esn: Long, name: String, description: String)
...
object Product {
  ...
  def findAll = products.toList.sortBy(_.esn)

  def findByESN(esn: Long) = products.find(_.esn == esn)

  def add(p: Product) = products += p
}
